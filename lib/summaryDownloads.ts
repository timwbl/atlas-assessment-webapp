...
function withPdfPageFragment(href: string, page?: number): string {
  const safePage = Math.floor(Number(page) || 0);
  if (!safePage || safePage < 1) return href;
  return `${href.split("#")[0]}#page=${safePage}`;
}
